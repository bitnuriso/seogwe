...
function autoDetectContent() {
   ...
   */
  const baseProtectionBounds = panelBounds || (
    foregroundHorizontal && foregroundVertical
      ? {
          left: foregroundHorizontal.start,
          right: foregroundHorizontal.end,
          top: foregroundVertical.start,
          bottom: foregroundVertical.end
        }
      : null
  );
  const peripheralProtection = collectPeripheralProtection(baseProtectionBounds);

  /*
   * 컷선이 잡힌 페이지에서는 전경 전체 극값을 다시 합치지 않는다.
   * 그렇지 않으면 컷 바깥 글자·먼지 하나 때문에 좌우 여백이 과도하게 늘어난다.
   * - panelBounds가 있으면 컷선을 기본값으로 사용
   * - 검증된 주변 보호 요소만 제한적으로 확장
   * - panelBounds가 없을 때만 기존 전경 bounds를 기본값으로 사용
   */
  const limitPeripheralExpansion = (base, protection) => {
    if (!base || !protection) return protection;

    const maxHorizontalExpansion = Math.max(2, Math.floor(gridWidth * 0.065));
    const maxVerticalExpansion = Math.max(2, Math.floor(gridHeight * 0.055));

    return {
      left: Math.max(protection.left, base.left - maxHorizontalExpansion),
      right: Math.min(protection.right, base.right + maxHorizontalExpansion),
      top: Math.max(protection.top, base.top - maxVerticalExpansion),
      bottom: Math.min(protection.bottom, base.bottom + maxVerticalExpansion),
      headerProtected: protection.headerProtected,
      footerProtected: protection.footerProtected
    };
  };

  const boundedPeripheralProtection = panelBounds
    ? limitPeripheralExpansion(panelBounds, peripheralProtection)
  ...
  const horizontal = panelBounds
    ? {
        start: Math.min(
          panelBounds.left,
          boundedPeripheralProtection?.left ?? panelBounds.left
        ),
        end: Math.max(
          panelBounds.right,
          boundedPeripheralProtection?.right ?? panelBounds.right
        )
      }
    : foregroundHorizontal
      ? {
          start: Math.min(
            foregroundHorizontal.start,
            boundedPeripheralProtection?.left ?? foregroundHorizontal.start
          ),
          end: Math.max(
            foregroundHorizontal.end,
            boundedPeripheralProtection?.right ?? foregroundHorizontal.end
          )
        }
      : null;
  ...
  const vertical = panelBounds
    ? {
        start: Math.min(
          panelBounds.top,
          boundedPeripheralProtection?.top ?? panelBounds.top
        ),
        end: Math.max(
          panelBounds.bottom,
          boundedPeripheralProtection?.bottom ?? panelBounds.bottom
        )
      }
    : foregroundVertical
      ? {
          start: Math.min(
            foregroundVertical.start,
            boundedPeripheralProtection?.top ?? foregroundVertical.start
          ),
          end: Math.max(
            foregroundVertical.end,
            boundedPeripheralProtection?.bottom ?? foregroundVertical.end
          )
        }
      : null;
  ...
  if (boundedPeripheralProtection?.headerProtected) detectNotes.push("상단 제목 보호");
  if (boundedPeripheralProtection?.footerProtected) detectNotes.push("페이지 번호 보호");
  if (boundedPeripheralProtection && !boundedPeripheralProtection.headerProtected && !boundedPeripheralProtection.footerProtected) detectNotes.push("비정형 컷 보호");
