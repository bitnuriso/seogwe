...
function renderMergeRows(){
  ...
  box.innerHTML=mergeRows.map((row,index)=>`<div class="row" data-merge-row="${index}" style="padding:8px 0;border-bottom:1px solid #333">
      <strong style="width:30px">${index+1}</strong>
      <span title="${esc(row.name)}" style="flex:1;min-width:190px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(row.name)}</span>
      <label>그룹 <input class="merge-group" type="number" min="1" value="${row.group??''}" placeholder="-" style="width:68px"></label>
      <label>앞 <input class="merge-first" type="number" min="0" value="${row.skip_first}" style="width:58px">장</label>
      <label>뒤 <input class="merge-last" type="number" min="0" value="${row.skip_last}" style="width:58px">장</label>
    </div>`).join("");
  box.querySelectorAll("[data-merge-row]").forEach(el=>{
    const index=Number(el.dataset.mergeRow);
    el.querySelector(".merge-group").addEventListener("input",e=>{
...
async function refreshMergeFiles(){
  ...
  try{
    ...
    mergeRows=mergeFilesCache.map(name=>previous.get(name)||{name,group:null,skip_first:0,skip_last:0});
    renderMergeRows();
...
async function mergeInputPdfs(){
  ...
  if(!groups.length){statusBox.textContent="병합할 PDF에 그룹 번호를 입력하세요.";return;}
...
document.getElementById("clearGroups").addEventListener("click",()=>{mergeRows.forEach(row=>row.group=null);renderMergeRows();});
refreshMergeFiles();
