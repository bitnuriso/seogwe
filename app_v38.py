...
#status,#importStatus,#mergeStatus{padding:8px 0;min-height:22px;white-space:pre-wrap}
.merge-row{display:grid;grid-template-columns:34px minmax(150px,1fr) 78px 78px;gap:6px;align-items:center;width:100%;padding:6px 0;border-bottom:1px solid #333}.merge-row input[type=number]{width:78px;min-width:0}.merge-head{font-size:12px;color:#999}.merge-list{max-height:310px;overflow:auto;width:100%}.toolbar{position:sticky;bottom:0;z-index:10;background:#181818;border-top:1px solid #333}.toolbar button{flex:1;min-width:90px}
...
<details><summary>IN 폴더 전체 자동 크롭</summary>
<div class="row" style="margin-top:10px">
<label>각 파일 마지막 <input id="autoBatchSkipLast" class="small" type="number" min="0" value="0">페이지 제거</label>
<label><input id="autoBatchDeleteSource" type="checkbox"> 완료 후 원본 삭제</label>
<button id="autoBatchStart" class="primary">처음부터 끝까지 자동감지 크롭</button>
<button id="autoBatchStop" disabled>자동 일괄 중지</button>
</div>
<div id="autoBatchStatus" class="meta"></div>
</details>
<details><summary>여러 PDF 조건 병합</summary>
<div class="row" style="margin-top:10px">
<input id="mergeName" type="text" placeholder="병합본 이름" value="병합본.pdf">
<button id="refreshMerge">목록 새로고침</button>
</div>
<div class="row">
<label>일괄 규칙: <input id="ruleStart" class="small" type="number" min="1" value="2">번째 파일부터</label>
<label>앞 <input id="ruleSkipFirst" class="small" type="number" min="0" value="1">장 제거</label>
<label>뒤 <input id="ruleSkipLast" class="small" type="number" min="0" value="0">장 제거</label>
<button id="applyMergeRule">목록에 적용</button>
</div>
<div class="merge-row merge-head"><span>사용</span><span>파일 순서</span><span>앞 제거</span><span>뒤 제거</span></div>
<div id="mergeList" class="merge-list"></div>
<div class="row"><button id="mergePdfs" class="primary">조건 적용해 병합본 만들기</button></div>
<div id="mergeStatus" class="meta"></div>
</details>
<div class="row">
...
function renderMergeList(){
  ...
  files.forEach((name,i)=>{
    ...
    row.innerHTML=`<input class="merge-use" type="checkbox" checked aria-label="사용">
      <span title="${name.replace(/"/g,"&quot;")}">${i+1}. ${name}</span>
      <input class="merge-first" type="number" min="0" value="0" aria-label="앞 제거">
      <input class="merge-last" type="number" min="0" value="0" aria-label="뒤 제거">`;
    list.appendChild(row);
  });
  if(!files.length) list.innerHTML='<div class="meta">병합할 PDF가 없습니다.</div>';
}

$("refreshMerge").onclick=async()=>{await loadFiles(false);renderMergeList();$("mergeStatus").textContent=`PDF ${files.length}개를 불러왔습니다.`};
$("applyMergeRule").onclick=()=>{
  const start=Math.max(1,Number($("ruleStart").value)||1);
  const first=Math.max(0,Number($("ruleSkipFirst").value)||0);
  const last=Math.max(0,Number($("ruleSkipLast").value)||0);
  [...$("mergeList").querySelectorAll(".merge-row")].forEach((row,i)=>{
    if(i+1>=start){row.querySelector(".merge-first").value=first;row.querySelector(".merge-last").value=last;}
  });
  $("mergeStatus").textContent=`${start}번째 파일부터 앞 ${first}장, 뒤 ${last}장 제거 규칙을 적용했습니다.`;
};
$("mergePdfs").onclick=async()=>{
  const items=[...$("mergeList").querySelectorAll(".merge-row")].filter(row=>row.querySelector(".merge-use").checked).map(row=>({
    name:row.dataset.name,
    skip_first:Math.max(0,Number(row.querySelector(".merge-first").value)||0),
    skip_last:Math.max(0,Number(row.querySelector(".merge-last").value)||0)
  }));
  if(items.length<2){$("mergeStatus").textContent="병합할 PDF를 2개 이상 선택하세요.";return;}
  $("mergePdfs").disabled=true;$("mergeStatus").textContent="조건을 적용해 병합 중…";
  try{
    const r=await fetch("/api/merge-pdfs",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items,output_name:$("mergeName").value||"병합본.pdf"})});
    const d=await r.json();if(!r.ok)throw new Error(d.error||"병합 실패");
    $("mergeStatus").textContent=`병합 완료: ${d.output_name} · ${d.pages}페이지${d.removed?` · ${d.removed}페이지 제거`:""}`;
    await loadFiles(false);renderMergeList();
    const mergedIndex=files.indexOf(d.output_name);if(mergedIndex>=0){index=mergedIndex;showCurrent();}
  }catch(e){$("mergeStatus").textContent=`병합 실패: ${e.message}`}
  finally{$("mergePdfs").disabled=false}
};
...
$("extract").onclick=async()=>{
  ...
  try{const r=await fetch("/api/import",{method:"POST",body:fd}),d=await r.json();if(!r.ok)throw new Error(d.error||"가져오기 실패");$("importStatus").textContent=`완료: PDF ${d.imported}개 추가${d.renamed?`, 중복명 ${d.renamed}개 변경`:""}${d.skipped?`, 건너뜀 ${d.skipped}개`:""}`;$("archive").value="";$("archivePassword").value="";index=0;await loadFiles(true);renderMergeList()}catch(e){$("importStatus").textContent=`실패: ${e.message}`}finally{$("extract").disabled=false}
};
$("save").onclick=async()=>{
  if(!files.length)return;
  if(!$("allPages").checked){
...
function automaticOutputName(filename) {
  const stem = filename.replace(/\.pdf$/i, "");
  return `${stem}_crop.pdf`;
}

async function loadFileForAutomaticBatch(fileIndex) {
  index = fileIndex;
  pdfDocument = null;
  pageIndex = 0;
  pageCount = 0;
...
async function detectAllRetainedPages(skipLast) {
  const retainedCount = pageCount - skipLast;
  if (retainedCount <= 0) {
    throw new Error(`마지막 ${skipLast}페이지를 제거하면 남는 페이지가 없습니다.`);
  }

  const detected = [];
  for (let page = 0; page < retainedCount; page++) {
    if (!autoBatchRunning) return null;
    pageIndex = page;
    await drawPreview(true);
    const ok = autoDetectContent();
    if (!ok) {
      throw new Error(`${page + 1}페이지 자동감지 실패`);
    }
    detected.push({...crop});
    $("autoBatchStatus").textContent =
      `자동감지 중: ${files[index]} · ${page + 1}/${retainedCount}페이지`;
    await batchSleep(20);
  }
  return detected;
}

$("autoBatchStart").onclick = async () => {
  if (autoBatchRunning || !files.length) return;

  const skipLast = Math.max(0, Number($("autoBatchSkipLast").value) || 0);
  const deleteSource = $("autoBatchDeleteSource").checked;
  const originalFiles = [...files];
  if (!confirm(`IN 폴더 PDF ${originalFiles.length}개를 자동감지 크롭할까요?\n각 파일 마지막 ${skipLast}페이지는 제외됩니다.`)) return;

  autoBatchRunning = true;
  $("autoBatchStart").disabled = true;
  $("autoBatchStop").disabled = false;
  let completed = 0;

  try {
    // 자동 일괄은 항상 현재 IN 목록의 첫 파일부터 처리한다.
    for (const filename of originalFiles) {
      if (!autoBatchRunning) break;
      await loadFiles(false);
      const fileIndex = files.indexOf(filename);
      if (fileIndex < 0) continue;

      $("autoBatchStatus").textContent =
        `파일 준비 중: ${completed + 1}/${originalFiles.length} · ${filename}`;
      await loadFileForAutomaticBatch(fileIndex);
      const detected = await detectAllRetainedPages(skipLast);
      if (!detected || !autoBatchRunning) break;
      ...
      const response = await fetch("/api/crop", {
        ...
        body: JSON.stringify({
          name: filename,
          output_name: automaticOutputName(filename),
          crop: detected[0],
          all_pages: false,
          page_crops: detected,
          skip_last: skipLast,
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || `${filename} 저장 실패`);
      completed++;
      $("autoBatchStatus").textContent =
        `완료 ${completed}/${originalFiles.length}: ${data.output_name}`;
    }

    $("autoBatchStatus").textContent = autoBatchRunning
      ? `자동 일괄 완료: ${completed}개`
      : `자동 일괄 중지: ${completed}개 완료`;
  } catch (error) {
    console.error(error);
    $("autoBatchStatus").textContent = `자동 일괄 실패: ${error.message}`;
  } finally {
    autoBatchRunning = false;
    $("autoBatchStart").disabled = false;
    $("autoBatchStop").disabled = true;
    await loadFiles(false);
    if (files.length) {
      index = 0;
      await showCurrent();
...
$("autoBatchStop").onclick = () => {
  autoBatchRunning = false;
  $("autoBatchStop").disabled = true;
  $("autoBatchStatus").textContent = "현재 단계 후 자동 일괄을 중지합니다.";
};
...
updateNumberControls();loadFiles().then(renderMergeList);
...
def api_merge_pdfs():
    try:
        ...
        if not isinstance(items, list) or len(items) < 2:
            raise ValueError("병합할 PDF를 2개 이상 선택하세요.")

        requested_name = str(body.get("output_name", "병합본.pdf"))
        output_stem = clean_filename(requested_name, "병합본")
        destination, _ = unique_path(IN_DIR, f"{output_stem}.pdf")
        writer = PdfWriter()
        removed = 0
        ...
        for item in items:
            if not isinstance(item, dict):
                raise ValueError("병합 조건 형식이 올바르지 않습니다.")
            name = str(item.get("name", ""))
            source = safe_input_path(name)
            reader = PdfReader(str(source))
            total = len(reader.pages)
            skip_first = max(0, int(item.get("skip_first", 0)))
            skip_last = max(0, int(item.get("skip_last", 0)))
            start = min(skip_first, total)
            end = max(start, total - skip_last)
            kept = end - start
            if kept <= 0:
                raise ValueError(f"{name}: 제거 조건 적용 후 남는 페이지가 없습니다.")
        ...
        with destination.open("wb") as output:
            writer.write(output)
        return jsonify(ok=True, output_name=destination.name, pages=len(writer.pages), removed=removed, sources=source_names)
    except Exception as error:
        return jsonify(error=str(error)), 400
...
def api_crop():
    try:
        ...
        skip_last = max(0, int(body.get("skip_last", 0) or 0))
        retained_count = len(reader.pages) - skip_last
        if retained_count <= 0:
            raise ValueError("마지막 페이지 제거 수가 전체 페이지 수 이상입니다.")
        ...
        if not apply_all:
            if not isinstance(page_crops, list) or len(page_crops) != retained_count:
                raise ValueError("페이지별 크롭 좌표 수가 남길 페이지 수와 다릅니다.")
        ...
        retained_pages = list(reader.pages[:retained_count])
        for page_index, page in enumerate(retained_pages):
        ...
        for page, rectangle in zip(retained_pages, crop_rectangles):
        ...
        return jsonify(ok=True, output_name=destination.name, source_deleted=delete_source, pages=retained_count, removed=skip_last)
    except Exception as error:
        return jsonify(error=str(error)), 400
