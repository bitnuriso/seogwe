...
function autoDetectContent() {
  ...
  /*
   * 2차 상단 메모 폴백.
   * 1차 run 분리가 실패했더라도, 페이지 상단의 희박한 전경 뒤에
   * 일정 길이의 저밀도 골짜기와 본문 밀도 상승이 이어지면 그 지점을
   * 본문 시작으로 확정한다. 제목처럼 본문에 붙어 있는 요소는 골짜기가
   * 없으므로 제거하지 않는다.
   */
  if (!topMarkRemoved) {
    ...
    for (let gy = 1; gy <= searchEnd; gy++) {
      ...
      if (quietStart >= 0 && gy - quietStart >= minQuietRun) {
        ...
        const cutoffIsNearTop = gy <= gridHeight * 0.30;

        if (topHasInk && bodyIsStable && cutoffIsNearTop) {
          fallbackStart = gy;
          break;
        }
      }
  ...
  /*
   * 상하 경계 전용 복구.
   * 좌우 결과는 절대 건드리지 않고, 선택된 좌우 범위 안에서 y방향으로
   * 연속되는 전경 행만 찾아 위/아래 경계를 바깥쪽으로 보정한다.
   * 메모 제거 이후의 cleaned 마스크를 사용하므로 상단 메모가 되살아나지 않는다.
   */
  const rescueVerticalEdges = bounds => {
    if (!bounds) return bounds;
    ...
    let rescuedTop = bounds.top;
    let lastActive = bounds.top;
    let gap = 0;
    for (let gy = bounds.top - 1; gy >= Math.max(contentStartRow, bounds.top - searchDistance); gy--) {
      if (rowInk(gy) >= minRowInk) {
        rescuedTop = gy;
        lastActive = gy;
        gap = 0;
      } else {
