...
<label><input id="autoEachPage" type="checkbox"> 페이지마다 단행본 자동감지</label>
<label><input id="lockAutoWidth" type="checkbox"> 자동감지 시 가로폭 고정</label>
...
function autoDetectContent() {
  ...
  /*
   * 단행본 스캔용 자동감지
   * 1) 페이지 가장자리에서 종이 배경색을 중앙값으로 추정
   * 2) 배경과 다른 픽셀을 저해상도 전경 마스크로 변환
   * 3) 작은 먼지/글자를 제거하고 연결 영역을 계산
   * 4) 컷 사이 여백 정도로 가까운 영역을 반복 병합
   * 5) 일정한 바깥 여백을 더한 뒤, 필요하면 가로폭은 유지하고 중심만 이동
   */
  ...
  const edgeSamples = [];
  const edgeDepth = Math.max(2, Math.floor(Math.min(width, height) * 0.018));
  const edgeStride = Math.max(3, Math.floor(Math.min(width, height) / 260));

  for (let x = 0; x < width; x += edgeStride) {
    for (let d = 1; d <= edgeDepth; d += edgeStride) {
      edgeSamples.push(pixelAt(x, Math.min(height - 1, d)));
      edgeSamples.push(pixelAt(x, Math.max(0, height - 1 - d)));
    }
  }
  for (let y = 0; y < height; y += edgeStride) {
    for (let d = 1; d <= edgeDepth; d += edgeStride) {
      edgeSamples.push(pixelAt(Math.min(width - 1, d), y));
      edgeSamples.push(pixelAt(Math.max(0, width - 1 - d), y));
    }
  }

  if (!edgeSamples.length) return false;
  ...
  const backgroundR = medianChannel(0);
  const backgroundG = medianChannel(1);
  const backgroundB = medianChannel(2);

  /* 휴대폰에서도 버티도록 약 350~520 셀 폭으로 축소한다. */
  const step = Math.max(2, Math.ceil(Math.max(width, height) / 480));
  const gridWidth = Math.ceil(width / step);
  const gridHeight = Math.ceil(height / step);
  const mask = new Uint8Array(gridWidth * gridHeight);

  const colorThreshold = 36;
  const darkThreshold = 218;
  ...
  for (let gy = 0; gy < gridHeight; gy++) {
    ...
    for (let gx = 0; gx < gridWidth; gx++) {
      ...
      if (distance > colorThreshold || luminance < darkThreshold) {
  ...
  /*
   * 한 셀짜리 먼지는 제거하고, 1셀 정도 끊어진 컷선은 이어 준다.
   * 완전한 morphology 대신 이웃 투표를 써서 계산량을 줄인다.
   */
    ...
    for (let gx = 1; gx < gridWidth - 1; gx++) {
      ...
      for (let oy = -1; oy <= 1; oy++) {
        for (let ox = -1; ox <= 1; ox++) {
          neighbors += mask[(gy + oy) * gridWidth + gx + ox];
        }
      }
      const current = mask[gy * gridWidth + gx];
      if ((current && neighbors >= 2) || (!current && neighbors >= 5)) {
        cleaned[gy * gridWidth + gx] = 1;
      }
    }
  }
  ...
  const components = [];
  const queueX = new Int32Array(cleaned.length);
  const queueY = new Int32Array(cleaned.length);
  ...
  for (let startY = 0; startY < gridHeight; startY++) {
    for (let startX = 0; startX < gridWidth; startX++) {
      ...
      while (head < tail) {
        ...
        for (let oy = -1; oy <= 1; oy++) {
          for (let ox = -1; ox <= 1; ox++) {
            ...
            if (!cleaned[nextIndex] || visited[nextIndex]) continue;
            visited[nextIndex] = 1;
            queueX[tail] = nx;
            queueY[tail++] = ny;
          }
      ...
      /* 작은 글자 한 덩이와 먼지는 버리되, 가는 컷선은 살린다. */
      const meaningful =
        area >= Math.max(10, gridWidth * gridHeight * 0.00016) &&
        (boxWidth >= gridWidth * 0.028 || boxHeight >= gridHeight * 0.025) &&
        density >= 0.018;
  ...
  if (!components.length) {
    crop = previousCrop;
    renderCrop();
    $("status").textContent = "자동감지 실패 · 이전 크롭을 유지했습니다.";
    return false;
  }

  const overlapLength = (a0, a1, b0, b1) =>
    Math.max(0, Math.min(a1, b1) - Math.max(a0, b0) + 1);

  const shouldMerge = (a, b) => {
    const gapX = Math.max(0, Math.max(a.x0, b.x0) - Math.min(a.x1, b.x1) - 1);
    const gapY = Math.max(0, Math.max(a.y0, b.y0) - Math.min(a.y1, b.y1) - 1);
    const overlapX = overlapLength(a.x0, a.x1, b.x0, b.x1);
    const overlapY = overlapLength(a.y0, a.y1, b.y0, b.y1);
    const widthA = a.x1 - a.x0 + 1;
    const widthB = b.x1 - b.x0 + 1;
    const heightA = a.y1 - a.y0 + 1;
    const heightB = b.y1 - b.y0 + 1;
    const overlapXRatio = overlapX / Math.max(1, Math.min(widthA, widthB));
    const overlapYRatio = overlapY / Math.max(1, Math.min(heightA, heightB));
    const maxGapX = Math.max(3, gridWidth * 0.032);
    const maxGapY = Math.max(3, gridHeight * 0.028);

    return (
      gapX <= maxGapX && overlapYRatio >= 0.10
    ) || (
      gapY <= maxGapY && overlapXRatio >= 0.10
    ) || (
      gapX <= maxGapX * 0.55 && gapY <= maxGapY * 0.55
    );
  };
  ...
  /* 너무 작은 고립 영역은 페이지 번호/먼지일 가능성이 높다. */
  const pageGridArea = gridWidth * gridHeight;
  boxes = boxes.filter(box => {
    const w = box.x1 - box.x0 + 1;
    const h = box.y1 - box.y0 + 1;
    return box.area >= pageGridArea * 0.0007 || (w >= gridWidth * 0.16 && h >= gridHeight * 0.08);
  });

  if (!boxes.length) {
    crop = previousCrop;
    renderCrop();
    $("status").textContent = "유효한 본문 영역이 없어 이전 크롭을 유지했습니다.";
    return false;
  }

  let minX = Math.min(...boxes.map(box => box.x0)) * step;
  let minY = Math.min(...boxes.map(box => box.y0)) * step;
  let maxX = Math.min(width, (Math.max(...boxes.map(box => box.x1)) + 1) * step);
  let maxY = Math.min(height, (Math.max(...boxes.map(box => box.y1)) + 1) * step);

  /* 일정한 결과를 위해 여백은 렌더 픽셀이 아니라 페이지 비율로 둔다. */
  const marginX = width * 0.018;
  const marginY = height * 0.013;
  minX = clamp(minX - marginX, 0, width);
  maxX = clamp(maxX + marginX, 0, width);
  minY = clamp(minY - marginY, 0, height);
  maxY = clamp(maxY + marginY, 0, height);

  const detectedWidthRatio = (maxX - minX) / width;
  const detectedHeightRatio = (maxY - minY) / height;

  /* 전면 삽화처럼 거의 페이지 전체가 전경이면 과도한 크롭을 하지 않는다. */
  if (detectedWidthRatio > 0.97 && detectedHeightRatio > 0.96) {
    crop = previousCrop;
    renderCrop();
    $("status").textContent = "전면 페이지로 판단 · 이전 크롭을 유지했습니다.";
    return false;
  }

  if (detectedWidthRatio < 0.25 || detectedHeightRatio < 0.25) {
    crop = previousCrop;
    renderCrop();
    $("status").textContent = "감지 영역이 비정상적으로 작아 이전 크롭을 유지했습니다.";
    return false;
  }
  ...
  if ($("lockAutoWidth").checked) {
    const fixedWidth = clamp(previousCrop.x1 - previousCrop.x0, 0.05, 1);
    const centerX = (detectedCrop.x0 + detectedCrop.x1) / 2;
    let x0 = centerX - fixedWidth / 2;
    let x1 = centerX + fixedWidth / 2;
    ...
    crop = {
      x0: clamp(x0, 0, 1 - fixedWidth),
      y0: detectedCrop.y0,
      x1: clamp(x1, fixedWidth, 1),
      y1: detectedCrop.y1
    };
  } else {
    crop = detectedCrop;
  ...
  renderCrop();
  $("status").textContent = $("lockAutoWidth").checked
    ? `단행본 감지 완료 · 가로폭 유지 · ${boxes.length}개 영역 병합`
    : `단행본 감지 완료 · ${boxes.length}개 영역 병합`;
  return true;
}
...
async function drawPreview(resetDocument=false){
  ...
  if(!$("allPages").checked && pageCrops[pageIndex]) crop={...pageCrops[pageIndex]};
  renderCrop();
  updateModeUI();
  const autoDetected =
    !$("allPages").checked &&
...
function clearFinished(){
  pdfDocument=null;pageIndex=0;pageCount=0;pageCrops={};
...
function showCurrent(){
  const has=files.length>0&&index<files.length;$("counter").textContent=has?`파일 ${index+1} / ${files.length}`:"파일 0 / 0";$("save").disabled=$("skip").disabled=$("prev").disabled=!has;
  if(has){
    stage.style.display = "";
    shade.style.display = "";
    ...
    stage.classList.remove("empty");$("original").textContent=`원본: ${files[index]}`;pdfDocument=null;pageIndex=0;pageCount=0;pageCrops={};drawPreview(true).catch(e=>$("status").textContent=`미리보기 실패: ${e.message}`)}else clearFinished();
}
...
$("allPages").addEventListener("change",()=>{pageIndex=0;pageCrops={};updateModeUI();if(files.length)drawPreview().catch(e=>$("status").textContent=`미리보기 실패: ${e.message}`)});
$("autoEachPage").addEventListener("change",()=>{if($("autoEachPage").checked&&!$("allPages").checked&&files.length){pageCrops={};drawPreview().catch(e=>$("status").textContent=`자동 감지 실패: ${e.message}`)}});
