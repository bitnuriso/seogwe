...
<a class="menu-card" href="/batch">
  ...
  <span class="menu-card-desc">IN 폴더의 여러 PDF를 자동 크롭하거나, 앞뒤 페이지를 제외해 하나로 병합합니다.</span>
</a>
</div>
<div class="row">
...
<div class="box"><h3 style="margin-top:0">IN PDF 병합</h3><div class="meta">아래 표시 순서대로 PDF를 합칩니다. 각 파일의 앞/뒤 페이지를 제외할 수 있으며, 병합 성공 후 원본을 IN에서 삭제할 수 있습니다.</div><div id="mergeFiles" class="file-list">목록 읽는 중…</div><div class="row" style="margin-top:10px"><label>각 파일 앞에서 <input id="mergeSkipFirst" type="number" min="0" value="0" style="width:70px"> 페이지 제외</label><label>뒤에서 <input id="mergeSkipLast" type="number" min="0" value="0" style="width:70px"> 페이지 제외</label></div><div class="row" style="margin-top:10px"><input id="mergeName" type="text" value="병합본" placeholder="새 PDF 이름"><label><input id="deleteMergedSources" type="checkbox" checked> 병합 성공 후 원본 삭제</label><button id="merge" type="button">IN에 병합본 만들기</button><button id="refreshMerge" class="secondary" type="button">목록 새로고침</button></div><div id="mergeStatus">대기 중</div></div>
...
async function refreshMergeFiles(){
  ...
  try{
    ...
    box.textContent=mergeFilesCache.length?mergeFilesCache.map((name,index)=>`${index+1}. ${name}`).join("\n"):"IN 폴더에 PDF가 없습니다.";
  }catch(error){box.textContent=`목록 오류: ${error?.message||error}`;}
}
async function mergeInputPdfs(){
  ...
  const skipFirst=Math.max(0,Number.parseInt(document.getElementById("mergeSkipFirst").value||"0",10)||0);
  const skipLast=Math.max(0,Number.parseInt(document.getElementById("mergeSkipLast").value||"0",10)||0);
  const deleteSources=document.getElementById("deleteMergedSources").checked;
  if(mergeFilesCache.length<2){statusBox.textContent="병합하려면 PDF가 2개 이상 필요합니다.";return;}
  button.disabled=true;
  statusBox.textContent=`${mergeFilesCache.length}개 PDF 병합 중…\n각 파일: 앞 ${skipFirst}장 · 뒤 ${skipLast}장 제외${deleteSources?" · 완료 후 원본 삭제":""}`;
  try{
    const response=await fetch("/api/merge-input",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({files:mergeFilesCache,output_name:outputName,skip_first:skipFirst,skip_last:skipLast,delete_sources:deleteSources})});
    const data=await response.json();
    if(!response.ok) throw new Error(data.error||"병합 실패");
    statusBox.textContent=`병합 완료: ${data.name}\n${data.files}개 파일 · ${data.pages}페이지\n각 파일 앞 ${data.skip_first}장 · 뒤 ${data.skip_last}장 제외${data.deleted_sources?`\n원본 ${data.deleted_sources}개 삭제 완료`:"\n원본 유지"}\n새 파일이 IN에 추가되어 바로 크롭할 수 있습니다.`;
...
def api_merge_input():
    """Merge PDFs from IN, optionally trimming each file, then optionally delete sources."""
    try:
        body: dict[str, Any] = request.get_json(force=True)
        requested_files = body.get("files")
        if not isinstance(requested_files, list):
            raise ValueError("병합할 PDF 목록이 올바르지 않습니다.")
        ...
        skip_first = max(0, int(body.get("skip_first", 0) or 0))
        ...
        try:
            ...
            # 병합 결과를 다시 열어 페이지 수를 확인한 뒤에만 원본을 삭제한다.
            ...
            if delete_sources:
                for source in source_paths:
                    # 새 병합본과 동일 경로일 가능성은 unique_path로 방지되지만 한 번 더 보호한다.
