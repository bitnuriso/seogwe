...
function autoDetectContent() {
  ...
  const collectPeripheralProtection = baseBounds => {
    ...
    const cornerWidth = Math.floor(gridWidth * 0.48);
      ...
      if (baseBounds) {
        ...
        const dy = component.maxY < baseBounds.top
          ...
          : component.minY > baseBounds.bottom
            ? component.minY - baseBounds.bottom
            : 0;
        const closeToMain = dx <= gridWidth * 0.045 && dy <= gridHeight * 0.045;
        const substantial =
  ...
  const horizontal = panelBounds
    ? {
        start: Math.min(
          panelBounds.left,
          foregroundHorizontal?.start ?? panelBounds.left,
          peripheralProtection?.left ?? panelBounds.left
        ),
        end: Math.max(
          panelBounds.right,
          foregroundHorizontal?.end ?? panelBounds.right,
          peripheralProtection?.right ?? panelBounds.right
        )
      }
    : foregroundHorizontal
      ? {
          start: Math.min(foregroundHorizontal.start, peripheralProtection?.left ?? foregroundHorizontal.start),
          end: Math.max(foregroundHorizontal.end, peripheralProtection?.right ?? foregroundHorizontal.end)
        }
      : null;
  ...
  const vertical = panelBounds
    ? {
        start: Math.min(
          panelBounds.top,
          foregroundVertical?.start ?? panelBounds.top,
          peripheralProtection?.top ?? panelBounds.top
        ),
        end: Math.max(
          panelBounds.bottom,
          foregroundVertical?.end ?? panelBounds.bottom,
          peripheralProtection?.bottom ?? panelBounds.bottom
        )
      }
    : foregroundVertical
      ? {
          start: Math.min(foregroundVertical.start, peripheralProtection?.top ?? foregroundVertical.start),
          end: Math.max(foregroundVertical.end, peripheralProtection?.bottom ?? foregroundVertical.end)
        }
      : null;
  ...
  if (peripheralProtection?.headerProtected) detectNotes.push("상단 제목 보호");
  if (peripheralProtection?.footerProtected) detectNotes.push("페이지 번호 보호");
  if (peripheralProtection && !peripheralProtection.headerProtected && !peripheralProtection.footerProtected) detectNotes.push("비정형 컷 보호");
