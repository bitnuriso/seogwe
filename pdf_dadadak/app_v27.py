...
function autoDetectContent() {
  ...
  const detectOuterPanelBounds = (startRow = 0) => {
    ...
    for (let gx = 0; gx <= gridWidth; gx++) {
      ...
      if (active) {
        ...
        if (strongSingle || stableBand) {
          verticalGroups.push({ start: groupStart, end, peak: groupPeak, x: groupPeakX });
        }
        groupStart = -1;
        groupPeak = 0;
        groupPeakX = -1;
    ...
    /* 가장 넓으면서 양쪽 선의 지지 길이가 충분한 군집 쌍을 선택한다. */
    let bestPair = null;
    for (let i = 0; i < verticalGroups.length - 1; i++) {
      for (let j = i + 1; j < verticalGroups.length; j++) {
        ...
        const span = rightGroup.x - leftGroup.x + 1;
        const widthRatio = span / gridWidth;
        if (widthRatio < 0.42 || widthRatio > 0.96) continue;
        const support = Math.min(leftGroup.peak, rightGroup.peak);
        const score = span * (support / contentHeight);
        if (!bestPair || score > bestPair.score) {
          bestPair = { left: leftGroup.x, right: rightGroup.x, score };
        }
      }
    }
    ...
    const left = bestPair.left;
    const right = bestPair.right;
