...
<details open><summary>PDF 일괄처리</summary>
<div class="row" style="margin-top:10px">
<label>각 파일 뒤에서 <input id="autoBatchSkipLast" class="small" type="number" min="0" value="0">페이지 삭제</label>
<button id="autoBatchStart" class="primary">일괄처리</button>
</div>
...
async function detectAllRetainedPages(skipLast) {
  ...
  for (let page = 0; page < retainedCount; page++) {
    ...
    $("autoBatchStatus").textContent =
      `처리 중: ${files[index]} · ${page + 1}/${retainedCount}페이지`;
    await batchSleep(20);
  }
  return detected;
}
...
$("autoBatchStart").onclick = async () => {
  ...
  try {
    ...
    $("autoBatchStatus").textContent = autoBatchRunning
      ? `일괄처리 완료: ${completed}개`
      : `일괄처리 중지: ${completed}개 완료`;
  } catch (error) {
    console.error(error);
    $("autoBatchStatus").textContent = `일괄처리 실패: ${error.message}`;
  } finally {
