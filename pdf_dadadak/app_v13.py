...
function autoDetectContent() {
  ...
  if (!isColorIllustration) {
    const activeRows = Array.from(
      smoothedRows,
      value => value >= minRowCount
    );
    
    // 상단 메모 후보는 페이지 최상단에 붙은 경우만 대상으로 한다.
    ...
    if (runs.length >= 2 && totalRowMass > 0) {
      ...
      const nearTop = first.end <= gridHeight * 0.003;
  ...
  /* 일단 edgeTrimRatio 임시로 반절로 줄임 */  
  const findBounds = (
    counts,
    minimum,
    startTrimRatio = 0.003,
    endTrimRatio = startTrimRatio
  ) => {
    const active = [];
    let total = 0;
    ...
    const findStart = () => {
      if (startTrimRatio <= 0) {
        return active.findIndex(value => value > 0);
      }

      const trim = total * startTrimRatio;
      let cumulative = 0;

      for (let i = 0; i < active.length; i++) {
        cumulative += active[i];
        if (cumulative >= trim && active[i] > 0) return i;
      }
  ...
  /*
   * 일반 페이지는 가장자리의 극소량 노이즈를 제외한다.
   * 컬러 삽화는 누적량 trim을 전혀 적용하지 않고,
   * 첫 유효 전경 행/열부터 마지막 전경 행/열까지 그대로 사용한다.
   * 삽화에서는 이미 고립 셀 제거를 거쳤으므로 추가 0.4% 절삭이 필요 없다.
   */
  const illustrationRowMinimum = Math.max(1, Math.floor(minRowCount * 0.5));
  const illustrationColumnMinimum = Math.max(1, Math.floor(minColumnCount * 0.5));

  const horizontal = isColorIllustration
    ? findBounds(smoothedColumns, illustrationColumnMinimum, 0, 0)
    : findBounds(smoothedColumns, minColumnCount, 0.004, 0.004);

  const vertical = isColorIllustration
    ? findBounds(smoothedRows, illustrationRowMinimum, 0, 0)
    : findBounds(smoothedRows, minRowCount, 0.004, 0.004);
