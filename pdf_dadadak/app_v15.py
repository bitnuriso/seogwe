...
function autoDetectContent() {
   ...
   * 첫 번째/두 번째 덩어리만 비교하지 않고, 가장 큰 본문 덩어리를 기준으로
   * 그 위에 떨어져 있는 작은 전경 덩어리를 모두 메모 후보로 본다.
   * 컬러 삽화에서도 삽화 위의 검은 메모가 있을 수 있으므로 동일하게 적용한다.
   */
  let topMarkRemoved = false;
  {
    ...
    if (runs.length >= 2 && totalRowMass > 0) {
      ...
      /* 가장 많은 전경량을 가진 덩어리를 본문/삽화 시작점으로 본다. */
      let mainIndex = 0;
      for (let i = 1; i < enrichedRuns.length; i++) {
        if (enrichedRuns[i].mass > enrichedRuns[mainIndex].mass) {
          mainIndex = i;
        }
      }

      const mainRun = enrichedRuns[mainIndex];
      const candidates = enrichedRuns.slice(0, mainIndex);
      ...
      if (candidates.length > 0) {
        ...
        const startsNearTop = candidateStart <= gridHeight * 0.12;
        const staysInTopArea = candidateEnd <= gridHeight * 0.24;
        const smallHeight = candidateHeight <= gridHeight * 0.11;
        const lowMass = candidateMass <= totalRowMass * 0.085;
        const enoughGap = detachedGap >= Math.max(4, Math.floor(gridHeight * 0.014));
        const mainIsSubstantial = mainRun.mass >= totalRowMass * 0.35;
        ...
        if (
          ...
          for (let row = 0; row < mainRun.start; row++) {
            smoothedRows[row] = 0;
            rowCounts[row] = 0;
          }
          topMarkRemoved = true;
   ...
   * 컬러 삽화도 동일한 감지 결과를 사용하고, 차이는 최종 margin만 0으로 둔다.
   */
