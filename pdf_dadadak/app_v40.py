...
async function runAutomaticBatch() {
  ...
  try {
    for (let fileIndex = 0; fileIndex < targets.length; fileIndex++) {
      const fileName = targets[fileIndex];
      $("autoBatchStatus").textContent =
        `파일 ${fileIndex + 1} / ${targets.length} 불러오는 중…\n${fileName}`;

      const url = `/api/pdf?name=${encodeURIComponent(fileName)}&t=${Date.now()}`;
      const documentForBatch = await pdfjsLib.getDocument(url).promise;
      const retainedCount = documentForBatch.numPages - skipLast;
      if (retainedCount <= 0) {
        failed++;
        $("autoBatchStatus").textContent =
          `건너뜀: ${fileName} · 삭제할 페이지 수가 전체 페이지 수 이상입니다.`;
        continue;
      }

      const detectedCrops = [];
      for (let pageNumber = 1; pageNumber <= retainedCount; pageNumber++) {
        $("autoBatchStatus").textContent =
          `파일 ${fileIndex + 1} / ${targets.length} · 페이지 ${pageNumber} / ${retainedCount}\n${fileName}`;

        const page = await documentForBatch.getPage(pageNumber);
        const base = page.getViewport({scale: 1});
        const cssWidth = Math.min(window.innerWidth - 24, 900);
      ...
      const response = await fetch("/api/crop", {
        ...
        body: JSON.stringify({
          name: fileName,
          output_name: `${stem}_crop.pdf`,
          crop: detectedCrops[0],
          all_pages: false,
          page_crops: detectedCrops,
          skip_last: skipLast,
          delete_source: false
        })
      });
    ...
    $("autoBatchStatus").textContent =
      `일괄처리 완료 · 성공 ${completed}개${failed ? ` · 건너뜀 ${failed}개` : ""}`;
  } catch (error) {
...
$("autoBatchStart").onclick = runAutomaticBatch;
