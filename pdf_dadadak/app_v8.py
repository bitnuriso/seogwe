...
<label><input id="fullBleedColor" type="checkbox" checked> 컬러 삽화는 여백 없이 전체 페이지</label>
...
function autoDetectContent() {
  ...
  /*
   * 컬러 전면 삽화 감지.
   * 채도가 있는 픽셀이 충분히 많고 페이지 넓게 분포하면
   * 고정 폭/높이보다 우선해서 전체 페이지를 사용한다.
   */
  if ($("fullBleedColor")?.checked) {
    ...
    const colorRatio = colored / Math.max(1, sampled);
    ...
    const fullBleedColorPage =
      colorRatio >= 0.055 &&
      colorSpreadX >= 0.68 &&
      colorSpreadY >= 0.68;

    if (fullBleedColorPage) {
      crop = { x0: 0, y0: 0, x1: 1, y1: 1 };
      renderCrop();
      $("status").textContent =
        `컬러 삽화 감지 · 여백 없이 전체 페이지 · 저장 이름: ${outputName()}`;
      return true;
    }
  }
  ...
  /* 전면 삽화도 정상 결과로 인정한다. 전체 페이지에 가까우면 작은 안전 여백만 둔다. */
  const marginX = detectedWidth > 0.90 ? 0.006 : 0.018;
  const marginY = detectedHeight > 0.92 ? 0.006 : 0.013;
  ...
  if ($("lockAutoWidth")?.checked) detectNotes.push("가로폭 고정");
  if ($("lockAutoHeight")?.checked) detectNotes.push("세로높이 고정");
