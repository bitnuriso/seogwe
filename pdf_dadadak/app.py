...
details{width:100%;background:#202020;border:1px solid #333;border-radius:12px;padding:8px 10px}summary{cursor:pointer;font-weight:700}.menu-card{display:block;width:100%;padding:14px 16px;border:1px solid #3f4f68;border-radius:12px;background:#1d2735;color:#fff;text-decoration:none;box-shadow:0 4px 14px rgba(0,0,0,.22)}.menu-card:hover{background:#243247;border-color:#5b7ca8}.menu-card-title{display:flex;align-items:center;gap:8px;font-weight:800;font-size:16px}.menu-card-desc{display:block;margin-top:5px;color:#b8c7dc;font-size:13px;line-height:1.4}
...
<div class="row">
<a class="menu-card" href="/batch-v6">
  <span class="menu-card-title">📚 PDF 일괄 크롭·병합</span>
</a>
</div>
<div class="row">
...
BATCH_HTML = r'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PDF 일괄처리 UI v6</title><style>body{font-family:system-ui,sans-serif;background:#111;color:#eee;padding:16px}main{max-width:920px;margin:auto}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}input,button,select{font:inherit;padding:10px;border:1px solid #666;border-radius:8px;background:#222;color:#eee}input[type=text]{flex:1;min-width:180px}button{background:#2563eb;border-color:#2563eb;cursor:pointer}button.secondary{background:#374151;border-color:#4b5563}button.danger{background:#7f1d1d;border-color:#991b1b}button:disabled{opacity:.5;cursor:default}#status,#mergeStatus{white-space:pre-wrap;margin-top:14px}.box{border:1px solid #444;border-radius:12px;padding:14px;margin-bottom:14px}.meta{font-size:13px;color:#aaa}.version{display:inline-block;margin-left:8px;padding:3px 8px;border-radius:999px;background:#14532d;color:#dcfce7;font-size:12px;vertical-align:middle}.file-list{margin-top:10px;padding:10px;background:#181818;border-radius:8px;max-height:520px;overflow:auto}.merge-item{display:grid;grid-template-columns:42px minmax(180px,1fr);gap:8px 10px;padding:12px 0;border-bottom:1px solid #333}.merge-item.excluded{opacity:.48}.merge-order{grid-row:1 / span 2;display:flex;flex-direction:column;gap:6px}.merge-order button{padding:7px 5px;min-width:38px}.merge-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:700}.merge-controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.merge-controls label{display:flex;gap:5px;align-items:center}.merge-controls select{min-width:110px}.merge-controls .new-group{width:92px}.exclude-label{padding:8px 10px;border:1px solid #555;border-radius:8px;background:#222}canvas{display:none}a{color:#9ec5ff}@media(max-width:640px){body{padding:10px}.merge-item{grid-template-columns:38px minmax(0,1fr)}.merge-controls{align-items:stretch}.merge-controls label:not(.exclude-label){width:100%}.merge-controls select,.merge-controls input{flex:1;min-width:0}.merge-controls button{flex:1}}</style></head><body><main><h2>PDF 일괄처리·병합 <span class="version">UI v6</span></h2>
<div class="box" style="border-color:#16a34a;background:#052e16"><strong>새 병합 UI가 실행 중입니다.</strong><div class="meta" style="color:#bbf7d0;margin-top:6px">빌드: batch-ui-v6-20260804 · 진단 주소: <a href="/version" target="_blank">/version</a></div></div>
<div class="box"><h3 style="margin-top:0">IN PDF 다중 그룹 병합</h3><div class="meta">각 PDF의 그룹 칸에 같은 번호를 입력하면 한 PDF로 묶입니다. 빈 칸은 제외됩니다. 파일 순서는 현재 목록 순서를 따릅니다.</div><div id="mergeFiles" class="file-list">목록 읽는 중…</div><div class="row" style="margin-top:10px"><button id="refreshMerge" class="secondary" type="button">IN 목록 새로고침</button><button id="clearGroups" class="secondary" type="button">그룹 입력 지우기</button></div><div class="row" style="margin-top:10px"><input id="mergeName" type="text" value="병합본" placeholder="결과 파일 기본 이름"><label>시작 번호 <input id="mergeStartNumber" type="number" min="0" value="1" style="width:90px"></label><label>자리수 <input id="mergeDigits" type="number" min="1" max="8" value="3" style="width:70px"></label></div><div class="row" style="margin-top:10px"><label><input id="deleteMergedSources" type="checkbox" checked> 병합 성공 후 사용한 원본 삭제</label><button id="merge" type="button">그룹별 PDF 한 번에 만들기</button></div><div id="mergeStatus">대기 중</div></div>
<div class="box"><h3 style="margin-top:0">IN 폴더 전체 자동 크롭</h3><div class="row"><label>각 파일 뒤에서 <input id="skipLast" type="number" min="0" value="0" style="width:70px"> 페이지 삭제</label><button id="start" type="button">일괄처리</button><button id="stop" type="button" disabled style="background:#b91c1c;border-color:#b91c1c">일괄 중지</button></div><div id="status">대기 중</div></div><p><a href="/">기존 화면으로 돌아가기</a></p><canvas id="canvas"></canvas><div id="detectorStatus" hidden></div><input id="lockAutoWidth" type="checkbox" hidden><input id="lockAutoHeight" type="checkbox" hidden><script type="module">
let pdfjsLib = null;
async function ensurePdfJs(){
  if(pdfjsLib) return pdfjsLib;
  try{
...
async function getFiles(){const r=await fetch("/api/files",{cache:"no-store"});if(!r.ok)throw new Error("IN 폴더 목록을 읽지 못했습니다.");const d=await r.json();return Array.isArray(d.files)?d.files:[];}
function knownGroups(){return [...new Set(mergeRows.map(row=>row.group).filter(value=>Number.isInteger(value)&&value>0))].sort((a,b)=>a-b);}
function moveMergeRow(index,direction){
  const target=index+direction;
  if(target<0||target>=mergeRows.length)return;
  [mergeRows[index],mergeRows[target]]=[mergeRows[target],mergeRows[index]];
  renderMergeRows();
}
function setRowGroup(index,value){
  const parsed=parseInt(value,10);
  mergeRows[index].group=Number.isFinite(parsed)&&parsed>0?parsed:null;
  renderMergeRows();
}
function renderMergeRows(){
  const box=document.getElementById("mergeFiles");
  if(!mergeRows.length){box.innerHTML='<div class="meta">IN 폴더에 PDF가 없습니다.</div>';return;}
  const groups=knownGroups();
  box.innerHTML=mergeRows.map((row,index)=>{
    const options=['<option value="">그룹 없음</option>',...groups.map(group=>`<option value="${group}" ${row.group===group?'selected':''}>그룹 ${group}</option>`)].join('');
    return `<div class="merge-item ${row.excluded?'excluded':''}" data-merge-row="${index}">
      <div class="merge-order">
        <button class="move-up secondary" type="button" ${index===0?'disabled':''} title="위로 이동">▲</button>
        <button class="move-down secondary" type="button" ${index===mergeRows.length-1?'disabled':''} title="아래로 이동">▼</button>
      </div>
      <div class="merge-name" title="${esc(row.name)}">${index+1}. ${esc(row.name)}</div>
      <div class="merge-controls">
        <label>그룹 선택 <select class="merge-group-select">${options}</select></label>
        <label>새 그룹 <input class="new-group" type="number" min="1" placeholder="번호"></label>
        <button class="apply-group secondary" type="button">추가·선택</button>
        <label>앞 <input class="merge-first" type="number" min="0" value="${row.skip_first}" style="width:64px">장</label>
        <label>뒤 <input class="merge-last" type="number" min="0" value="${row.skip_last}" style="width:64px">장</label>
        <label class="exclude-label"><input class="merge-excluded" type="checkbox" ${row.excluded?'checked':''}> 이번 병합에서 제외</label>
      </div>
    </div>`;
  }).join("");
  box.querySelectorAll("[data-merge-row]").forEach(el=>{
    const index=Number(el.dataset.mergeRow);
    el.querySelector(".move-up").addEventListener("click",()=>moveMergeRow(index,-1));
    el.querySelector(".move-down").addEventListener("click",()=>moveMergeRow(index,1));
    el.querySelector(".merge-group-select").addEventListener("change",e=>{
      const value=e.target.value;
      mergeRows[index].group=value===""?null:Math.max(1,parseInt(value,10)||1);
    });
    el.querySelector(".apply-group").addEventListener("click",()=>{
      const input=el.querySelector(".new-group");
      const value=Math.max(1,parseInt(input.value||"0",10)||0);
      if(!value){input.focus();return;}
      setRowGroup(index,value);
    });
    el.querySelector(".new-group").addEventListener("keydown",e=>{
      if(e.key==="Enter"){e.preventDefault();el.querySelector(".apply-group").click();}
    });
    el.querySelector(".merge-first").addEventListener("input",e=>mergeRows[index].skip_first=Math.max(0,parseInt(e.target.value||"0",10)||0));
    el.querySelector(".merge-last").addEventListener("input",e=>mergeRows[index].skip_last=Math.max(0,parseInt(e.target.value||"0",10)||0));
    el.querySelector(".merge-excluded").addEventListener("change",e=>{
      mergeRows[index].excluded=e.target.checked;
      el.classList.toggle("excluded",e.target.checked);
    });
  });
}
async function refreshMergeFiles(){
  const previous=new Map(mergeRows.map(row=>[row.name,{...row}]));
  try{
    mergeFilesCache=await getFiles();
    const existingNames=new Set(mergeFilesCache);
    const kept=mergeRows.filter(row=>existingNames.has(row.name));
    const known=new Set(kept.map(row=>row.name));
    const added=mergeFilesCache.filter(name=>!known.has(name)).map(name=>previous.get(name)||{name,group:null,skip_first:0,skip_last:0,excluded:false});
    mergeRows=[...kept,...added].map(row=>({...row,excluded:Boolean(row.excluded)}));
    renderMergeRows();
  }catch(error){document.getElementById("mergeFiles").textContent=`목록 오류: ${error?.message||error}`;}
}
async function mergeInputPdfs(){
  const button=document.getElementById("merge");
  const statusBox=document.getElementById("mergeStatus");
  const outputName=document.getElementById("mergeName").value.trim()||"병합본";
  const startNumber=Math.max(0,parseInt(document.getElementById("mergeStartNumber").value||"0",10)||0);
  const digits=Math.max(1,Math.min(8,parseInt(document.getElementById("mergeDigits").value||"3",10)||3));
  const deleteSources=document.getElementById("deleteMergedSources").checked;
  const grouped=new Map();
  for(const row of mergeRows){
    if(row.excluded||row.group==null)continue;
    if(!grouped.has(row.group))grouped.set(row.group,[]);
    grouped.get(row.group).push({name:row.name,skip_first:Math.max(0,row.skip_first|0),skip_last:Math.max(0,row.skip_last|0)});
  }
  const groups=[...grouped.entries()].sort((a,b)=>a[0]-b[0]).map(([group,items])=>({group,items}));
  if(!groups.length){statusBox.textContent="병합할 PDF의 그룹을 선택하세요.";return;}
  const tooSmall=groups.find(entry=>entry.items.length<2);
  if(tooSmall){statusBox.textContent=`그룹 ${tooSmall.group}에는 PDF가 ${tooSmall.items.length}개뿐입니다. 그룹마다 2개 이상 선택하세요.`;return;}
  button.disabled=true;
  statusBox.textContent=`${groups.length}개 그룹 병합 중…${deleteSources?"\n완료 후 사용한 원본 삭제":""}`;
  try{
    const response=await fetch("/api/merge-input-groups",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({groups,output_name:outputName,start_number:startNumber,digits,delete_sources:deleteSources})});
    const data=await response.json();
    if(!response.ok)throw new Error(data.error||"병합 실패");
    const lines=data.outputs.map(item=>`${item.name} · ${item.files}개 파일 · ${item.pages}페이지`);
    statusBox.textContent=`병합 완료 (${data.outputs.length}개)\n${lines.join("\n")}${data.deleted_sources?`\n원본 ${data.deleted_sources}개 삭제 완료`:"\n원본 유지"}`;
    await refreshMergeFiles();
  }catch(error){statusBox.textContent=`병합 실패: ${error?.message||error}`;}
  finally{button.disabled=false;}
}
document.getElementById("merge").addEventListener("click",mergeInputPdfs);
document.getElementById("refreshMerge").addEventListener("click",refreshMergeFiles);
document.getElementById("clearGroups").addEventListener("click",()=>{mergeRows.forEach(row=>{row.group=null;row.excluded=false;});renderMergeRows();});
refreshMergeFiles();
function blankPageRatio(){
  ...
  for(let y=0;y<height;y+=step){
    for(let x=0;x<width;x+=step){
      ...
      sampled++;
      if(a>16&&(r<242||g<242||b<242)) foreground++;
    }
  }
  return sampled?foreground/sampled:1;
}

async function detectPage(pdf,pageNumber,previousCrop=null){
  let page=null;
  let renderTask=null;
  try{
    ...
    const foregroundRatio=blankPageRatio();
    ...
    if(previousCrop&&foregroundRatio<0.0015){
      const resultArea=Math.max(0,result.x1-result.x0)*Math.max(0,result.y1-result.y0);
      const previousArea=Math.max(0,previousCrop.x1-previousCrop.x0)*Math.max(0,previousCrop.y1-previousCrop.y0);
      if(resultArea<previousArea*0.88){
        return {...previousCrop,_reusedForBlank:true};
      }
    }
    return result;
  }finally{
    ...
    for(let fi=0;fi<files.length;fi++){
      ...
      try{
        ...
        for(let p=1;p<=keep;p++){
          ...
          const detected=await detectPage(pdf,p,crops.length?crops[crops.length-1]:null);
          if(detected._reusedForBlank){
            delete detected._reusedForBlank;
            status.textContent=`${fi+1}/${files.length} · ${p}/${keep} 빈 페이지: 이전 크롭 유지\n${name}\n완료 ${completed} · 건너뜀 ${skipped} · 실패 ${failures.length}`;
          }
          crops.push(detected);
        }
        if(stopRequested) break;
...
def version_page():
    response = jsonify(
        ...
        batch_ui="UI v6",
        has_reorder_buttons=True,
        has_exclude_control=True,
        has_group_selector=True,
    )
...
@app.get("/batch-v6")
def batch_page():
    response = Response(render_template_string(BATCH_HTML))
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["Expires"] = "0"
    response.headers["X-PDFCrop-Build"] = BUILD_ID
    return response
...
@app.post("/api/merge-input")
def api_merge_input():
    """Merge selected PDFs from IN with per-file trim rules, then optionally delete sources."""
    try:
        body: dict[str, Any] = request.get_json(force=True)
        requested_items = body.get("items")
        if not isinstance(requested_items, list):
            raise ValueError("병합 목록이 올바르지 않습니다.")

        items: list[dict[str, Any]] = []
        seen: set[str] = set()
        for value in requested_items:
            if not isinstance(value, dict):
                raise ValueError("병합 항목 형식이 올바르지 않습니다.")
            name = str(value.get("name", ""))
            if not name:
                raise ValueError("PDF 파일명이 비어 있습니다.")
            if name in seen:
                raise ValueError(f"같은 PDF를 두 번 병합할 수 없습니다: {name}")
            safe_input_path(name)
            items.append({
                "name": name,
                "skip_first": max(0, int(value.get("skip_first", 0) or 0)),
                "skip_last": max(0, int(value.get("skip_last", 0) or 0)),
            })
            seen.add(name)
        ...
        delete_sources = bool(body.get("delete_sources", False))
        output_stem = clean_filename(str(body.get("output_name", "병합본")), "병합본")
        destination, was_renamed = unique_path(IN_DIR, f"{output_stem}.pdf")
        ...
        try:
            for item in items:
                name = item["name"]
                skip_first = item["skip_first"]
                skip_last = item["skip_last"]
                source = safe_input_path(name)
                source_paths.append(source)
                reader = PdfReader(str(source))
                if reader.is_encrypted:
                    try:
                        if reader.decrypt("") == 0:
                            raise ValueError(f"암호화된 PDF는 병합할 수 없습니다: {name}")
                    except Exception as error:
                        raise ValueError(f"암호화된 PDF는 병합할 수 없습니다: {name}") from error
                ...
                if start_page >= end_page:
                    raise ValueError(
                        f"제외 조건 때문에 남는 페이지가 없습니다: {name} "
                        f"(전체 {page_count}장, 앞 {skip_first}장, 뒤 {skip_last}장 제외)"
                    )
            ...
            if len(verification.pages) != total_pages:
                ...
                raise ValueError("병합본 검증에 실패했습니다. 원본은 삭제하지 않았습니다.")
...
@app.post("/api/merge-input-groups")
def api_merge_input_groups():
    """Merge multiple ordered PDF groups in one request, numbering outputs consecutively."""
    created_paths: list[Path] = []
    temporary_paths: list[Path] = []
    try:
        body: dict[str, Any] = request.get_json(force=True)
        requested_groups = body.get("groups")
        if not isinstance(requested_groups, list) or not requested_groups:
            raise ValueError("병합 그룹 목록이 올바르지 않습니다.")

        output_stem = clean_filename(str(body.get("output_name", "병합본")), "병합본")
        start_number = max(0, int(body.get("start_number", 0) or 0))
        digits = max(1, min(8, int(body.get("digits", 3) or 3)))
        delete_sources = bool(body.get("delete_sources", False))
        groups: list[list[dict[str, Any]]] = []
        all_seen: set[str] = set()

        for group_value in requested_groups:
            if not isinstance(group_value, dict) or not isinstance(group_value.get("items"), list):
                raise ValueError("병합 그룹 형식이 올바르지 않습니다.")
            items: list[dict[str, Any]] = []
            for value in group_value["items"]:
                if not isinstance(value, dict):
                    raise ValueError("병합 항목 형식이 올바르지 않습니다.")
                name = str(value.get("name", ""))
                if not name:
                    raise ValueError("PDF 파일명이 비어 있습니다.")
                if name in all_seen:
                    raise ValueError(f"같은 PDF를 여러 그룹에 넣을 수 없습니다: {name}")
                safe_input_path(name)
                all_seen.add(name)
                items.append({
                    "name": name,
            ...
            if len(items) < 2:
                raise ValueError("각 병합 그룹에는 PDF가 2개 이상 필요합니다.")
            groups.append(items)

        outputs: list[dict[str, Any]] = []
        source_paths: list[Path] = []
        for offset, items in enumerate(groups):
            number = str(start_number + offset).zfill(digits)
            destination, was_renamed = unique_path(IN_DIR, f"{output_stem}_{number}.pdf")
            temporary = destination.with_name(f".{destination.name}.merge.tmp")
            ...
            try:
                for item in items:
                    source = safe_input_path(item["name"])
                    source_paths.append(source)
                    reader = PdfReader(str(source))
                    if reader.is_encrypted:
                        try:
                            if reader.decrypt("") == 0:
                                raise ValueError(f"암호화된 PDF는 병합할 수 없습니다: {item['name']}")
                        except Exception as error:
                            raise ValueError(f"암호화된 PDF는 병합할 수 없습니다: {item['name']}") from error
                    page_count = len(reader.pages)
                    start_page = min(item["skip_first"], page_count)
                    end_page = max(start_page, page_count - item["skip_last"])
                    if start_page >= end_page:
                        raise ValueError(f"제외 조건 때문에 남는 페이지가 없습니다: {item['name']}")
                    for page_index in range(start_page, end_page):
                        writer.add_page(reader.pages[page_index])
                        total_pages += 1
                if total_pages < 1:
                ...
                if len(PdfReader(str(destination)).pages) != total_pages:
                    raise ValueError("병합본 검증에 실패했습니다. 원본은 삭제하지 않았습니다.")
                outputs.append({"name": destination.name, "renamed": was_renamed, "files": len(items), "pages": total_pages})
            finally:
                try:
                    writer.close()
                except Exception:
        ...
        deleted_sources = 0
        if delete_sources:
            created_resolved = {path.resolve() for path in created_paths}
            for source in dict.fromkeys(source_paths):
                if source.resolve() in created_resolved:
                    continue
                source.unlink()
                deleted_sources += 1

        return jsonify(ok=True, outputs=outputs, deleted_sources=deleted_sources)
    except Exception as error:
...
if __name__ == "__main__":
    print("=" * 64)
    print(f"PDFCrop 빌드: {BUILD_ID}")
    print(f"실행 파일: {SOURCE_FILE}")
    print(f"입력 폴더: {IN_DIR}")
    print(f"출력 폴더: {OUT_DIR}")
    print("확인 주소: http://127.0.0.1:8001/version")
    print("배치 화면: http://127.0.0.1:8001/batch-v6")
    print("=" * 64)
