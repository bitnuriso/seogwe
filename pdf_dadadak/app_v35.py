...
def api_crop():
    try:
        ...
        target_width = max(right - left for left, bottom, right, top in crop_rectangles)
        ...
        for page, rectangle in zip(reader.pages, crop_rectangles):
            ...
            offset_x = (target_width - crop_width) / 2
            offset_y = (target_height - crop_height) / 2

            # merge 시 크롭 밖의 원본이 다시 나타나지 않도록 cropbox와 mediabox를
            # 같은 사각형으로 제한한 뒤, 공통 크기의 빈 페이지 중앙에 합친다.
