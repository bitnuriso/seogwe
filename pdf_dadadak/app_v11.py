...
<label><input id="lockAutoWidth" type="checkbox"> 현재 감지 가로폭 고정</label>
<label><input id="lockAutoHeight" type="checkbox"> 현재 감지 세로높이 고정</label>
<label><input id="ignoreTopMarks" type="checkbox" checked> 상단 낙서·메모 제외</label>
...
let lockedAutoWidth=null,lockedAutoHeight=null;
...
function autoDetectContent() {
  ...
  /*
   * 컬러 삽화 여부만 판정한다.
   * 여기서 전체 페이지로 강제하지 않고, 아래의 일반 외곽 감지 결과를 그대로 사용한다.
   * 컬러 삽화로 판정되면 추가 margin만 0으로 적용한다.
   */
  const colorStep = Math.max(2, Math.ceil(Math.max(width, height) / 500));
  let sampledColorPixels = 0;
  let coloredPixels = 0;
  let minColorX = width;
  let minColorY = height;
  let maxColorX = -1;
  let maxColorY = -1;

  for (let y = 0; y < height; y += colorStep) {
    for (let x = 0; x < width; x += colorStep) {
      const i = (y * width + x) * 4;
      ...
      /* 종이의 약한 누런 기운은 컬러 삽화로 세지 않는다. */
      if (chroma >= 24 && luma >= 18 && luma <= 242) {
        coloredPixels++;
        minColorX = Math.min(minColorX, x);
        minColorY = Math.min(minColorY, y);
        maxColorX = Math.max(maxColorX, x);
        maxColorY = Math.max(maxColorY, y);
      }
    }
  }

  const colorRatio = coloredPixels / Math.max(1, sampledColorPixels);
  const colorSpreadX = maxColorX >= minColorX
    ? (maxColorX - minColorX + colorStep) / width
    : 0;
  const colorSpreadY = maxColorY >= minColorY
    ? (maxColorY - minColorY + colorStep) / height
    : 0;
  ...
  /*
   * 상단 여백에 따로 떨어진 작은 낙서/메모 제거.
   * 색 자체로 판단하지 않고 다음 조건을 모두 만족할 때만 제외한다.
   * - 페이지 상단 25% 안에 있음
   * - 아래 본문과 충분한 빈 간격이 있음
   * - 높이와 전경량이 모두 작음
   * 제목이나 상단 컷처럼 큰 요소는 유지한다.
   */
  let topMarkRemoved = false;
  if (!isColorIllustration && $("ignoreTopMarks")?.checked) {
    const activeRows = Array.from(
      smoothedRows,
      value => value >= minRowCount
    );
    ...
    const totalRowMass = smoothedRows.reduce((sum, value) => sum + value, 0);
    const minimumGap = Math.max(5, Math.floor(gridHeight * 0.022));

    if (runs.length >= 2 && totalRowMass > 0) {
      const first = runs[0];
      const second = runs[1];
      const firstHeight = first.end - first.start + 1;
      let firstMass = 0;
      for (let row = first.start; row <= first.end; row++) {
        firstMass += smoothedRows[row];
      }

      const detachedGap = second.start - first.end - 1;
      const nearTop = first.end <= gridHeight * 0.25;
      const smallHeight = firstHeight <= gridHeight * 0.075;
      const lowMass = firstMass <= totalRowMass * 0.055;

      if (nearTop && smallHeight && lowMass && detachedGap >= minimumGap) {
        for (let row = 0; row < second.start; row++) {
          smoothedRows[row] = 0;
          rowCounts[row] = 0;
        }
        topMarkRemoved = true;
  ...
  const horizontal = findBounds(smoothedColumns, minColumnCount, 0.004);
  const vertical = findBounds(smoothedRows, minRowCount, 0.004);
  ...
  /* 컬러 삽화는 감지된 실제 외곽에 맞닿게 추가 여백을 0으로 둔다. */
  const marginX = isColorIllustration
    ? 0
    : (detectedWidth > 0.90 ? 0.006 : 0.018);
  const marginY = isColorIllustration
    ? 0
  ...
  if (!isColorIllustration && $("lockAutoWidth")?.checked) {
    const fixedWidth = clamp(lockedAutoWidth ?? (previousCrop.x1 - previousCrop.x0), 0.05, 1);
    const centerX = (x0 + x1) / 2;
    x0 = centerX - fixedWidth / 2;
    x1 = centerX + fixedWidth / 2;
  ...
  if (!isColorIllustration && $("lockAutoHeight")?.checked) {
    const fixedHeight = clamp(lockedAutoHeight ?? (previousCrop.y1 - previousCrop.y0), 0.05, 1);
    const centerY = (y0 + y1) / 2;
    y0 = centerY - fixedHeight / 2;
  ...
  if (!isColorIllustration && $("lockAutoWidth")?.checked) detectNotes.push("가로폭 고정");
  if (!isColorIllustration && $("lockAutoHeight")?.checked) detectNotes.push("세로높이 고정");
  const detectNote = detectNotes.length ? ` · ${detectNotes.join(" · ")}` : "";
  $("status").textContent = isColorIllustration
    ? `컬러 삽화 외곽 자동 선택 · 마진 0 · 저장 이름: ${outputName()}`
...
function syncAutoLockFromCurrentCrop(axis) {
  if (axis === "width") {
    lockedAutoWidth = clamp(crop.x1 - crop.x0, 0.05, 1);
    return;
  }
  lockedAutoHeight = clamp(crop.y1 - crop.y0, 0.05, 1);
}

$("lockAutoWidth").addEventListener("change", () => {
  if ($("lockAutoWidth").checked) {
    syncAutoLockFromCurrentCrop("width");
    $("status").textContent = `현재 자동감지 폭을 고정했습니다 · 저장 이름: ${outputName()}`;
  } else {
    lockedAutoWidth = null;
    $("status").textContent = `자동감지 가로폭 고정을 해제했습니다 · 저장 이름: ${outputName()}`;
  }
});

$("lockAutoHeight").addEventListener("change", () => {
  if ($("lockAutoHeight").checked) {
    syncAutoLockFromCurrentCrop("height");
    $("status").textContent = `현재 자동감지 높이를 고정했습니다 · 저장 이름: ${outputName()}`;
  } else {
    lockedAutoHeight = null;
    $("status").textContent = `자동감지 세로높이 고정을 해제했습니다 · 저장 이름: ${outputName()}`;
  }
});
