...
<div class="box"><h3 style="margin-top:0">IN PDF 병합</h3><div class="meta">PDF마다 앞/뒤 제외 페이지를 따로 설정합니다. 행 순서가 병합 순서이며, + 추가 / 삭제로 바로 구성할 수 있습니다.</div><div id="mergeFiles" class="file-list">목록 읽는 중…</div><div class="row" style="margin-top:10px"><button id="addMergeRow" class="secondary" type="button">+ PDF 추가</button><button id="refreshMerge" class="secondary" type="button">IN 목록 새로고침</button></div><div class="row" style="margin-top:10px"><input id="mergeName" type="text" value="병합본" placeholder="새 PDF 이름"><label><input id="deleteMergedSources" type="checkbox" checked> 병합 성공 후 사용한 원본 삭제</label><button id="merge" type="button">IN에 병합본 만들기</button></div><div id="mergeStatus">대기 중</div></div>
...
function renderMergeRows(){
  ...
  if(!mergeRows.length){box.innerHTML='<div class="meta">병합할 PDF가 없습니다. “+ PDF 추가”를 누르세요.</div>';return;}
  box.innerHTML=mergeRows.map((row,index)=>{
    const options=mergeFilesCache.map(name=>`<option value="${esc(name)}"${name===row.name?' selected':''}>${esc(name)}</option>`).join("");
    return `<div class="row" data-merge-row="${index}" style="padding:8px 0;border-bottom:1px solid #333">
      <strong style="width:26px">${index+1}</strong>
      <select class="merge-file" style="flex:1;min-width:190px;padding:9px;border-radius:8px;background:#222;color:#eee;border:1px solid #666">${options}</select>
      ...
      <button class="remove-merge-row" type="button" style="background:#7f1d1d;border-color:#991b1b">삭제</button>
    </div>`;
  }).join("");
  box.querySelectorAll("[data-merge-row]").forEach(el=>{
    const index=Number(el.dataset.mergeRow);
    el.querySelector(".merge-file").addEventListener("change",e=>mergeRows[index].name=e.target.value);
    ...
    el.querySelector(".remove-merge-row").addEventListener("click",()=>{mergeRows.splice(index,1);renderMergeRows();});
  });
}
function addMergeRow(){
  if(!mergeFilesCache.length){document.getElementById("mergeStatus").textContent="IN 폴더에 PDF가 없습니다.";return;}
  const used=new Set(mergeRows.map(row=>row.name));
  const name=mergeFilesCache.find(candidate=>!used.has(candidate))||mergeFilesCache[0];
  mergeRows.push({name,skip_first:0,skip_last:0});
  renderMergeRows();
}
async function refreshMergeFiles(){
  const previous=mergeRows.map(row=>({...row}));
  try{
    mergeFilesCache=await getFiles();
    mergeRows=previous.filter(row=>mergeFilesCache.includes(row.name));
    if(!mergeRows.length) mergeRows=mergeFilesCache.map(name=>({name,skip_first:0,skip_last:0}));
    renderMergeRows();
...
async function mergeInputPdfs(){
  ...
  const items=mergeRows.map(row=>({name:row.name,skip_first:Math.max(0,row.skip_first|0),skip_last:Math.max(0,row.skip_last|0)}));
  if(items.length<2){statusBox.textContent="병합하려면 PDF 행이 2개 이상 필요합니다.";return;}
  if(new Set(items.map(item=>item.name)).size!==items.length){statusBox.textContent="같은 PDF가 두 번 선택됐습니다. 중복 행을 삭제하거나 다른 파일을 선택하세요.";return;}
  button.disabled=true;
  statusBox.textContent=`${items.length}개 PDF 병합 중…${deleteSources?"\n완료 후 사용한 원본 삭제":""}`;
  try{
    const response=await fetch("/api/merge-input",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items,output_name:outputName,delete_sources:deleteSources})});
    const data=await response.json();
    if(!response.ok) throw new Error(data.error||"병합 실패");
    statusBox.textContent=`병합 완료: ${data.name}\n${data.files}개 파일 · ${data.pages}페이지${data.deleted_sources?`\n원본 ${data.deleted_sources}개 삭제 완료`:"\n원본 유지"}\n새 파일이 IN에 추가되어 바로 크롭할 수 있습니다.`;
    mergeRows=[];
...
document.getElementById("addMergeRow").addEventListener("click",addMergeRow);
