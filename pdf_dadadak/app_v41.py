...
async function runAutomaticBatchSafe() {
  const status = document.getElementById("autoBatchStatus");
  const button = document.getElementById("autoBatchStart");
  const input = document.getElementById("autoBatchSkipLast");
  if (!status || !button || !input) return;

  const skipLast = Math.max(0, Number(input.value) || 0);
  ...
  try {
    for (let fileIndex = 0; fileIndex < targets.length; fileIndex++) {
      ...
      try {
        status.textContent = `파일 ${fileIndex + 1}/${targets.length} 불러오는 중: ${fileName}`;
        const batchPdf = await pdfjsLib.getDocument(`/api/pdf?name=${encodeURIComponent(fileName)}&t=${Date.now()}`).promise;
        const retainedCount = batchPdf.numPages - skipLast;
        if (retainedCount <= 0) throw new Error("삭제 후 남는 페이지가 없습니다.");

        const detectedCrops = [];
        for (let pageNumber = 1; pageNumber <= retainedCount; pageNumber++) {
          status.textContent = `${fileIndex + 1}/${targets.length} · ${pageNumber}/${retainedCount}: ${fileName}`;
          const page = await batchPdf.getPage(pageNumber);
          const base = page.getViewport({scale: 1});
          const dpr = Math.min(window.devicePixelRatio || 1, 2);
          const cssWidth = Math.max(320, Math.min(window.innerWidth - 24, 900));
        ...
        failures.push(`${fileName}: ${fileError.message}`);
      }
    }
    status.textContent = failures.length
      ? `완료 ${completed}개 · 실패 ${failures.length}개\n${failures.join("\n")}`
      : `일괄처리 완료 · ${completed}개`;
  } finally {
    button.disabled = false;
    try { await loadFiles(false); } catch (e) { console.error(e); }
  }
}

window.addEventListener("load", () => {
  const button = document.getElementById("autoBatchStart");
  if (button) button.addEventListener("click", runAutomaticBatchSafe);
});
