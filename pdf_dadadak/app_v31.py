  ...
  const detectOuterPanelBounds = (startRow = 0) => {
    ...
    for (let i = 0; i < verticalGroups.length - 1; i++) {
      for (let j = i + 1; j < verticalGroups.length; j++) {
        ...
        const hasFixedXRun = Boolean(leftGroup.fixedXRun || rightGroup.fixedXRun);
        /* x 고정형 런은 짧은 돌출 컷용이므로 일반 16% 선 필터를 그대로 적용하지 않는다. */
        const minimumSupportRatio = hasFixedXRun ? 0.035 : 0.16;
