...
function autoDetectContent() {
  ...
  const panelBounds = isIllustration ? null : detectOuterPanelBounds();
  ...
  const horizontal = panelBounds
    ...
    : findBounds(smoothedColumns, minColumnCount, 0.004);
  const vertical = panelBounds
    ? { start: panelBounds.top, end: panelBounds.bottom }
    : findBounds(smoothedRows, minRowCount, 0.004);
  ...
  if (topMarkRemoved && !panelBounds) detectNotes.push("상단 낙서 제외");
  ...
  $("status").textContent = isIllustration
    ? `삽화 외곽 자동 선택 · 마진 0 · 저장 이름: ${outputName()}`
