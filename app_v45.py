...
const status=document.getElementById("status"),startButton=document.getElementById("start"),skipInput=document.getElementById("skipLast");
...
async function detectPage(pdf,pageNumber){const page=await pdf.getPage(pageNumber);const base=page.getViewport({scale:1});const scale=Math.min(2,1200/base.width);const viewport=page.getViewport({scale});canvas.width=Math.max(1,Math.floor(viewport.width));canvas.height=Math.max(1,Math.floor(viewport.height));await page.render({canvasContext:ctx,viewport}).promise;crop={x0:.03,y0:.03,x1:.97,y1:.97};autoDetectContent();return {...crop};}
async function run(){const skipLast=Math.max(0,Math.floor(Number(skipInput.value)||0));startButton.disabled=true;let completed=0;const failures=[];try{const files=await getFiles();if(!files.length){status.textContent="IN 폴더에 PDF가 없습니다.";return;}for(let fi=0;fi<files.length;fi++){const name=files[fi];try{status.textContent=`${fi+1}/${files.length} 파일 여는 중\n${name}`;await ensurePdfJs();const pdf=await pdfjsLib.getDocument(`/api/pdf?name=${encodeURIComponent(name)}&t=${Date.now()}`).promise;const keep=pdf.numPages-skipLast;if(keep<1)throw new Error("삭제 후 남는 페이지가 없습니다.");const crops=[];for(let p=1;p<=keep;p++){status.textContent=`${fi+1}/${files.length} · ${p}/${keep} 자동감지\n${name}`;crops.push(await detectPage(pdf,p));}const stem=name.replace(/\.pdf$/i,"");const r=await fetch("/api/crop",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,output_name:`${stem}_crop.pdf`,all_pages:false,page_crops:crops,page_limit:keep,delete_source:false})});const d=await r.json();if(!r.ok)throw new Error(d.error||"저장 실패");completed++;}catch(e){failures.push(`${name}: ${e.message}`);}}status.textContent=failures.length?`완료 ${completed}개 · 실패 ${failures.length}개\n${failures.join("\n")}`:`일괄처리 완료 · ${completed}개`;}catch(e){status.textContent=`일괄처리 실패: ${e.message}`;}finally{startButton.disabled=false;}}
