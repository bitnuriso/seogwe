  ...
  const expandPanelBoundsForStaggeredPanels = bounds => {
    ...
    for (let sy = contentStartRow; sy < gridHeight; sy++) {
      for (let sx = 0; sx < gridWidth; sx++) {
        ...
        if (strongVerticalPanel) {
          expanded.left = Math.min(
            expanded.left,
            Math.max(bounds.left - maxHorizontalExpansion, minX)
          );
          expanded.right = Math.max(
            expanded.right,
            Math.min(bounds.right + maxHorizontalExpansion, maxX)
          );
        }
