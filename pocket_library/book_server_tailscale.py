...
# Tailscale IPv4 주소는 CGNAT 대역(100.64.0.0/10)을 사용한다.
...
class BookHandler(SimpleHTTPRequestHandler):
    ...
    def is_client_allowed(self):
        ...
        try:
            return ipaddress.ip_address(client_ip) in TAILSCALE_NETWORK
        except ValueError:
            return False
    ...
    def serve_download(self, request_path, query, head_only=False):
        ...
        try:
            with open(local_path,"rb") as source:
                ...
                while remaining > 0:
                    ...
                    now = time.monotonic()
                    absolute_sent = start + sent_this_request

                    if (now - last_progress_update >= 0.25 or absolute_sent - last_reported_sent >= 4 * 1024 * 1024 ):

                    	PROGRESS.update(token, absolute_sent)
                    	last_progress_update = now
                    	last_reported_sent = absolute_sent
        ...
        speed_mbps = (
	    sent_this_request / elapsed / 1024 / 1024
	    if elapsed > 0
	    else 0
		)
...
def main():
    ...
    print(f"[BOOK] LAN 주소: http://192.168.219.126:{args.port}/")
    print(f"[BOOK] Tailscale 주소: http://100.117.50.59:{args.port}/")
